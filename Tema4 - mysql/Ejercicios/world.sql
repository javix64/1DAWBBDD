...
-- Dump completed on 2019-12-06 12:12:23


show tables;

-- 1.- Show three of the five columns of the City table. There are lots of rows in the table. We limit the query to the first 10 rows.
select name, district, population from city limit 10;
-- 2.- The LIMIT clause can be followed by two numbers. The first one is the offset and the second one is the number of rows to display. Show rows 16-20.
select name, district, population,id from city limit 15, 4;
...
select name,population from city where name like 'Kal%';
...
SELECT Name, District FROM city WHERE District = 'New York';
